...
Tree* Fdelete(int value, Tree* t)
{
	...
	if (value < t->element)
	{
		t->left = Fdelete(value, t->left);
	}
	else if (value > t->element)
	{
		t->right = Fdelete(value, t->right);
	}
	else if (t->left && t->right)
	{
		tmp_cell = find_min(t->right);
		t->element = tmp_cell->element;
		t->right = Fdelete(t->element, t->right);
	}
	else
	{
		tmp_cell = t;
...
int main()
{
	...
	root = Fdelete(14, root);
	print_ascii_tree(root);

	printf("\n\nAfter deleting a node (13) with left child..\n");
	root = Fdelete(13, root);
	print_ascii_tree(root);
